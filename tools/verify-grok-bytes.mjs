...
// 2a. 新路径：请求体就是图字节本身
const postRaw = (bytes, qs, contentType = 'image/png') => fetch(BASE + '/dvp/grok/save?' + qs, {
  method: 'POST',
  headers: { 'Content-Type': contentType },
  body: bytes,
})

const rawRes = await postRaw(BIG, 'batch=' + encodeURIComponent(batchId) + '&index=1&slug=demo-png')
...
const rawJpegRes = await postRaw(SMALL, 'batch=' + encodeURIComponent(batchId) + '&index=2&slug=demo-jpg&ext=.jpg', 'image/jpeg')
...
const jsonRes = await fetch(BASE + '/dvp/grok/save', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json; charset=utf-8' },
  body: JSON.stringify({ batchId, index: 3, slug: 'demo-json', base64: SMALL.toString('base64') }),
})
...
const emptyRes = await postRaw(Buffer.alloc(0), 'batch=' + encodeURIComponent(batchId) + '&index=4&slug=empty')
...
const badRes = await postRaw(BIG, 'batch=../evil&index=5&slug=escape')
...
// ── 3. 会话文本：取图配方不得再教 agent 搬 base64 ───────────────────────────
...
const driverDoc = typeof host.driverDoc === 'function'
  ? host.driverDoc({ dir: batchDirForRecipe, batchId, entries, grokUrl: 'https://grok.com/', createdAt: '2026-09-15T00:00:00.000Z', count: entries.length, options: { clarity: '1080p', aspect: '3:4' } })
  : null
