...
// 批次隔离回归：派发请求必须把批次带上，agent 存图时才能进对目录（P1 数据覆盖）。
// 参数名与路由实际读的对齐：raw bytes 模式下批次走 URL 参数 `batch=`（`batchId=` 只存在于 JSON 体里，
// agent 现在一律走 raw 直传，所以派发请求里必须写 `batch=`，写错就是"图落进最新一批"的老毛病）。
